...
namespace RASAT_Fizik_Motoru_STM32_ile_senkron3._1
{
    ...
    public static class FM_Fizik_Sabitler
    {
        ...
        public const double ROTASYONEL_SONUMLEME_KATSAYISI = 0.05; // Uydunun kendi etrafında fırıldak gibi dönmesini engelleyen sürtünme katsayısı
        ...
        // Quadcopter Kol Geometrisi (X Konfigürasyonu - 3D Konumlandırma)
        public const double MOTOR_KOL_UZUNLUGU_M = 0.07;       // Kolun gerçek uzunluğu (70 mm)
        public const double MOTOR_KOL_IZDUSUM_M = MOTOR_KOL_UZUNLUGU_M * 0.7071;    // 45 derece X-config için sin/cos izdüşümü (L * 0.7071)
        public const double MOTOR_Z_KAYMASI_M = 0.035;         // Pervane düzleminin Ağırlık Merkezine (CoM) dikey mesafesi (+Z yukarı)
        public const double d = FM_Fizik_Sabitler.MOTOR_KOL_IZDUSUM_M;
        public const double h = FM_Fizik_Sabitler.MOTOR_Z_KAYMASI_M;
        public static Vektor_t R_M1_body = new Vektor_t(d, d, h, Kordinat_Sistemi_t.GOVDE_BODY);   // Sağ-Ön
        public static Vektor_t R_M2_body = new Vektor_t(-d, d, h, Kordinat_Sistemi_t.GOVDE_BODY);  // Sol-Ön
        public static Vektor_t R_M3_body = new Vektor_t(-d, -d, h, Kordinat_Sistemi_t.GOVDE_BODY); // Sol-Arka
        public static Vektor_t R_M4_body = new Vektor_t(d, -d, h, Kordinat_Sistemi_t.GOVDE_BODY);  // Sağ-Arka

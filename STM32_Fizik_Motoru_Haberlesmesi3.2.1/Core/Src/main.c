...
//MODEL UYDU TEKNOFEST 2026 FİZİK MOTORU İLE SİMÜLASYON KODU 3.2.1
...
Alim_Veri_t alinacak;        // DMA'nın doğrudan yazdığı RAW tampon
Alim_Veri_t islenecek;       // Main döngüsünün güvenle okuyacağı GÜVENLİ tampon (Double Buffer)
Iletim_Veri_t ayarlanacak;
Iletim_Veri_t iletilecek;

volatile uint8_t paket_hazir_bayragi = 0;
volatile uint8_t hata_bayragi = 0;
volatile uint8_t tekrar_bayragi = 0;
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  if (paket_hazir_bayragi == 1)
	  {
		  ...
		  if (islenecek.paket.index < previndex) {
			  previndex = islenecek.paket.index;
		  }

		  zaman = (float)(islenecek.paket.index) * TIME_STEP;
		  dt = (float)(islenecek.paket.index - previndex) * TIME_STEP;
		  ...
		  uint32_t delta_ticks = islenecek.paket.index - previndex;
		...
		ayarlanacak.paket.index  = islenecek.paket.index;
	  ...
	  // ===============================================================
	  // DURUM 3: FİZİK MOTORUNDAN ALARM PAKETİ GELİRSE EN SON PAKETİ BİR DAHA YOLLA
	  // ===============================================================
	  if (tekrar_bayragi == 1)
	  {
		  tekrar_bayragi = 0; // Kilitlenmeyi çözen düzeltme
		  HAL_UART_Transmit(&huart2, iletilecek.buffer, sizeof(Iletim_Paketi_t), 50);
	  }
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART2)
    {
        ...
        if (alinacak.paket.header == PAKET_HEADER)
        {
            ...
            {
                if(alinacak.paket.rezerve == 0xF0)
                {
                	tekrar_bayragi = 1;
                }
                else
                {
                    // Double Buffering: Temiz veriyi islenecek tampona kopyala
                    memcpy(&islenecek, &alinacak, sizeof(Alim_Paketi_t));

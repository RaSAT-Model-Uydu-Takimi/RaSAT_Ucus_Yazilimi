...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Main program body - TIM3 Donanımsal Watchdog Entegrasyonu
  ******************************************************************************
  */
...
#define BAYRAK_APAM  (1U << 0)
#define BAYRAK_SEP   (1U << 1)
#define BAYRAK_SGM   (1U << 2)
#define BAYRAK_BUZZ  (1U << 3)

// Hata Kodu Maskeleri (Bit 4 - 7)
#define HATA_NACK    (0x0FU << 4)
...
typedef struct __attribute__((packed)){
    ...
    uint16_t mi3;
    uint16_t mi4;
    uint8_t  durum_bayraklari;
    uint8_t  checksum;
} Iletim_Paketi_t; // 16 Bayt
...
#define PERIOD_400HZ (1.0f / 400.0f)
#define PERIOD_100HZ (1.0f / 100.0f)
#define PERIOD_10HZ  (1.0f / 10.0f)
#define PERIOD_1HZ   (1.0f / 1.0f)
...
Alim_Veri_t alinacak;
Iletim_Veri_t ayarlanacak;
Iletim_Veri_t iletilecek;

volatile uint8_t paket_hazir_bayragi = 0;
volatile uint8_t hata_bayragi = 0;
...
int main(void)
{
  ...
  // 1. Standart 52 baytlık sabit boyutlu DMA alımını başlatıyoruz (USB boşluklarına toleranslı)
  HAL_UART_Receive_DMA(&huart2, alinacak.buffer, sizeof(Alim_Paketi_t));

  // 2. Donanımsal Bekçi Köpeği Sayacını (TIM3 - 5ms Periyotlu) Başlatıyoruz
  ...
  while (1)
  {
    ...
    if (hata_bayragi == 1)
    {
        ...
        alarm_paketi.paket.index  = previndex;
...
// 1. BA�?ARILI PAKET ALIMI KESMESİ (Standart DMA 52 Baytı Tamamladığında Çalışır)
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART2)
    {
        // Paket eksiksiz ulaştı! Donanımsal Timer sayacını sıfırla (Köpeği besle!)
        ...
        // Bir sonraki 52 baytlık paket için DMA'yı yeniden kur
        HAL_UART_Receive_DMA(&huart2, alinacak.buffer, sizeof(Alim_Paketi_t));
    }
}

// 2. DONANIMSAL TIMER WATCHDOG KESMESİ (Her 5 Milisaniyede Bir Tetiklenir)
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == TIM3)
    {
        // DMA sayacını okuyalım. (Kalan bayt sayısı)
        uint16_t kalan_bayt = __HAL_DMA_GET_COUNTER(huart2.hdmarx);

        // Eğer 52 bayttan az kalmışsa (yani veri gelmeye başlamış ama 5 ms geçmesine rağmen bitmemişse)
        // DEADLOCK (Kilitlenmiş veya Yarıda Kopmuş Paket) Tespiti!
        if (kalan_bayt < sizeof(Alim_Paketi_t) && kalan_bayt > 0)
        {
            HAL_UART_AbortReceive(&huart2); // Kilitli DMA hattını zorla kes
            memset(alinacak.buffer, 0, sizeof(Alim_Paketi_t));

            hata_bayragi = 1; // Ana döngüye NACK (Alarm) göndermesini söyle

            // Temiz bir hatla sıfırdan 52 bayt beklemeye başla
            HAL_UART_Receive_DMA(&huart2, alinacak.buffer, sizeof(Alim_Paketi_t));
        }
    }
}
